...
| [`NOTES.md`](NOTES.md) | 60 条学习笔记,格式是「现象 → 规则」成对 |
...
> 讲长上下文成本,不要说「长上下文贵」,要说
> **「它的成本随轮数平方增长,而 RAG 把那个平方项的常数压小了 43 倍」**。
...
> **所以答案是按查询类型路由,不是二选一。**
> 而做这个判断的前提是**能分辨查询类型**——没有分类评测集,
> 你看到的只是一个 76% 的总分,看不出哪一类该走哪条路。
...
**三个杠杆里两个的结论是「别做」或「看情况」。**
这不是失败——**能说清楚为什么不做,比做了一堆说不清效果的改动更接近真实工程。**
