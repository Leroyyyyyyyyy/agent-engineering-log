...
> 这三行是 hit@k 口径测的。指标后来改过(见下文),但 **@5 上两种口径同值**——
> `cross_file` 在 k=5 时中文 0/3、英文 q27 全覆盖,coverage 和 hit 给出同一个数,
> 所以这张表的数字在新口径下依然成立。前两行的索引已经不存在,无法重测。
...
| [`NOTES.md`](NOTES.md) | 43 条学习笔记,格式是「现象 → 规则」成对 |
